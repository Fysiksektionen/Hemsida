...
class ExtendedListSerializer(serializers.ListSerializer):
    """Overrides serializers.ListSerializer to allow for changes to child serializer individual to object serialized."""
    ...
    def to_internal_value(self, data):
        ...
        for i, item in enumerate(data):
            try:
                item = self.prep_child_from_dict(i, item)
                self.child.is_valid(raise_exception=True)
            except ValidationError as exc:
                errors.append(exc.detail)
            else:
                ret.append(self.child.validated_data)
                errors.append({})
    ...
    def to_representation(self, data):
        ...
        for i, item in enumerate(iterable):
            item = self.prep_child_from_instances(i, item)
    ...
    def create(self, validated_data):
        ...
        for i, attrs in enumerate(validated_data):
            attrs = self.prep_child_from_dict(i, attrs)
            ret.append(self.child.create(attrs))

        return ret

    def save(self, **kwargs):
        super(ExtendedListSerializer, self).save(**kwargs)

    def prep_child_from_instances(self, index, instance):
        """Method run before each usage of self.child when you have the object data"""
        self.child.order = index
        self.child.instance = instance
        if hasattr(self.child, '_validated_data'):
            del self.child._validated_data
        del self.child.fields

        return instance

    def prep_child_from_dict(self, index, item_data):
        """Method run before each usage of self.child when you have the dict data"""
        self.child.order = index
        self.child.initial_data = item_data
        if hasattr(self.child, '_validated_data'):
            del self.child._validated_data
        del self.child.fields
